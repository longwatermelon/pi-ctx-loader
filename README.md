...
When the extension injects files, it also moves the global agent-dir AGENTS.md (which pi core places at the start of the `<project_context>` block) to the very end of the system prompt, after the injected files. Projects without the config file keep pi's default ordering.

Also includes `writing-style.ts`: an opt-in extension that prepends `~/.pi/agent/writing-style.md` (wrapped in `<writing-style-reminder>` tags) to every user message. Toggle with `/writing-style`; per-session, default off. A missing, unreadable, or empty style file is skipped silently, and slash commands are left untouched.

Install by copying the extensions to pi's global extensions dir:

```bash
cp context-loader.ts writing-style.ts ~/.pi/agent/extensions/
```
